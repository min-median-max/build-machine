...
AIRDATA에 OS별 job을 가지고 타깃을 못박지 않은 `release.yml`을 만들었습니다. 각 플랫폼이 자기 job을 재현했습니다:
...
각 release 리허설은 자기가 만든 패키지를 열었습니다. 디스크 이미지를 마운트해 앱을 꺼냈고, `.deb`의 메타데이터와 내용을 읽었고, Windows 설치 파일이 실행 이미지인지 실행하지 않고 확인했습니다. 어느 것도 시스템 설치를 하지 않으며 각 영수증이 그렇게 적습니다.

이 과정에서 찾은 결함은 전부 이 머신의 코드에 있었습니다:

- 재현기가 `runs-on`을 무시하고 모든 job의 단계를 모든 플랫폼에서 돌려, OS별 job을 가진 workflow는 아예 재현할 수 없었습니다. 스테이지 게이트도 이제 플랫폼별로 확인합니다. macOS만 테스트하고 Linux는 안 해도 검증을 통과하던 구멍이었습니다.
- macOS 디스크 이미지 번들러가 앱을 이미지 안으로 옮기므로, 패키지를 열기 전에 앱을 찾는 순서로는 release가 성립할 수 없었습니다. 이제 패키지를 먼저 엽니다.
- 패키지 검사 자체가 재작성에서 기록 없이 사라져 있었습니다. 세 플랫폼 모두 복원했습니다.
- Windows의 `canonicalize`가 확장 경로를 반환해, 해석된 경로를 해석하지 않은 기준과 비교하던 모든 포함 검사가 실패했습니다.
- `dpkg-deb` 목록은 앞 슬래시 없이 경로를 적는데 Linux 검사 초판이 그걸 몰랐습니다.

### 데스크톱 세션은 전제 조건이므로 머신이 마련합니다

빌드와 실행은 로그인된 사용자로 수행되는데, 막 설치했거나 재시작한 기계는 로그인 화면에 있고 `--current-user`가 붙을 세션이 없습니다. 그걸 사람이 손으로 고치고 있었는데, 그건 프로비저닝이 아닙니다.

이제 `setup-system`이 보장합니다. 데스크톱 사용자를 `machine.json`에 선언하고, 자동 로그인이 안 돼 있으면 설정하고, 아무도 로그인해 있지 않을 때만 디스플레이 매니저를 재시작합니다. 다시 실행하면 "No change", "No restart"입니다. `doctor`는 세션을 다른 요구사항과 같이 보고하고, 게스트 워커 빌드는 세션이 아예 필요 없어졌습니다 — 컴파일에 데스크톱이 필요하지 않으므로 그 사용자가 되어 빌드합니다.

화면이 꺼지거나 잠기지 않게 하는 것도 함께 합니다. 실행 확인은 화면을 보고 하는데, 아무도 앉아 있지 않은 기계는 몇 분 뒤 화면을 꺼버려 그 확인이 불가능해집니다.

그다음 Linux 게스트를 재시작하는 것으로 충분했습니다. 아무도 로그인하지 않았는데 세션이 스스로 돌아왔고, `build-machine run ~/Work/airdata --os linux`가 앱을 띄웠습니다. `.state/ubuntu-app.png`에 한국어 보드 화면이 렌더링된 모습이 있습니다. 세 운영체제 중 마지막으로 보지 못했던 것입니다.

이 기능이 의존하는 판정은 테스트로 고정했습니다. 원래 실수를 낳은 경우도 포함합니다 — 설정 파일은 항목들을 주석 처리된 예시로 담고 있고, 그 주석을 설정으로 읽는 것이 프로비저닝이 성공을 보고하면서 기계는 로그인 화면에 남아 있게 만드는 원인입니다.

### 수행하지 않은 것

- 어떤 패키지의 시스템 수준 설치. 각 리허설은 만든 패키지를 열어볼 뿐 설치하지 않습니다.
