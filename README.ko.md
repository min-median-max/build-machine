...
[Tauri 2 데스크톱 앱](GUI.ko.md)은 등록된 프로젝트를 중심으로 동작합니다. `+`로 Git 폴더를 등록하고 해당 항목을 선택해 빌드 대상 설정·빌드·실행을 합니다. 프로젝트별 옵션은 각각 저장합니다. 하단 **설정** 버튼에서 모든 프로젝트가 공유하는 Windows/Ubuntu/macOS 진단과 도구 자동 준비를 엽니다. 환경별 마지막 진단/준비 결과와 완료 시각은 앱을 다시 열어도 유지합니다. 연결 상태와 과거 도구 검사 결과는 구분해서 표시합니다. 지원 구조와 남은 프레임워크 검증 범위는 [프로젝트 빌드 조건](PROJECTS.ko.md)에 있습니다.
