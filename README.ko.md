...
[Tauri 2 데스크톱 앱](GUI.ko.md)은 등록된 프로젝트와 최근 빌드 결과·로그를 보여주는 대시보드로 열립니다. `+`로 Git 폴더를 등록한 뒤 항목을 선택해 빌드 대상을 설정하고 빌드·실행합니다. 프로젝트마다 자기 옵션을 유지합니다. 하단 **설정** 버튼은 모든 프로젝트가 공유하는 Windows/Ubuntu/macOS 진단과 자동 도구 준비를 엽니다. 지원하는 배치와 남은 프레임워크 범위는 [프로젝트 빌드 요구사항](PROJECTS.ko.md)에 있습니다.
